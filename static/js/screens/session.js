...
function renderOpenPracticeDetail(ctx, view) {
  ...
  ctx.refs.mainPanel.innerHTML = `
    <section class="screen-content">
      <div class="muted-sm" style="font-weight: 600; letter-spacing: 0.02em; text-transform: uppercase;">
        ${ctx.escapeHtml(view.header || "Тема")}
      </div>
  ...
  `;
  ctx.refs.mainPanel.querySelector("#open-practice-detail-actions").append(
    ctx.actionButton("Повернутися до списку тем", close, "block"),
  );
}
...
export function renderCurrentView(ctx) {
  ...
  if (view.screen === "open-practice-detail") return renderOpenPracticeDetail(ctx, view);
